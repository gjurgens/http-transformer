...
var handleRedirects = function(proxyRes, req, res, options) {
    if (
        ...
        console.log("Rewriting host for a '" + proxyRes.statusCode + "' redirect. " + proxyRes.headers.location + " => " + urlParser.format(handledRedirect));
...
module.exports = function(_options) {
    ...
    return function(req, res, next){
        ...
        if(req.transformedRequestParams.target && req.transformedRequestParams.target.host) {
            var url = (req.transformedRequestParams.target.protocol || req.protocol || "http") + "://" + req.transformedRequestParams.target.host + (req.transformedRequestParams.target.url || req.originalUrl || "/");

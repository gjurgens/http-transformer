    ...
    var transformer = function(req, res) {
        ...
        var addTransformers = function(transformers) {
            ...
            for(var currentTransformerIndex = 0; currentTransformerIndex < transformers.length; currentTransformerIndex++) {
                ...
                message += indent(4) + "Transformer #" + currentTransformerIndex + "\n";
                if(currentTransformer.type && currentTransformer.pattern && currentTransformer.pattern.length === 3 && currentTransformer.pattern[1].match(/^[^m]+m[^m]+$/)) {
                    message += currentTransformer.type ? indent(5) + "Type: " + currentTransformer.type + " (slow unbuffered handling, because use of 'm' flag in regexp)\n" : "";
                } else {
                ...
                message += currentTransformer.pattern ? indent(5) + "Pattern: " + currentTransformer.pattern.toString().replace("\n","\\n") + "\n" : "";
        ...
        if(req && req.transformedRequestParams && req.transformedRequestParams.history && req.transformedRequestParams.history.length > 0) {
            ...
            for(var requestTransformerIndex = 0; requestTransformerIndex < req.transformedRequestParams.history.length; requestTransformerIndex++) {
                ...
                if(requestTransformer.matchers.length || requestTransformer.transformers.length) {
        ...
        if(res && res.transformedResponseParams && res.transformedResponseParams.history && res.transformedResponseParams.history.length > 0) {
            ...
            for(var responseTransformerIndex = 0; responseTransformerIndex < res.transformedResponseParams.history.length; responseTransformerIndex++) {
                var responseTransformer = res.transformedResponseParams.history[responseTransformerIndex];
                if(responseTransformer.matchers.length || responseTransformer.transformers.length) {
                    message += indent(2) + "TRANSFORMER '" + responseTransformer.name + "' (" + (responseTransformer.applies ? "" : "NOT ") + "APPLIED)\n";
                    message += addMatchers(responseTransformer.matchers, 3);
                    message += addTransformers(responseTransformer.transformers);
                    message += indent(3) + "\n";
                }
            }
        }
